			...
			*/
    		...
    		var BIKO1=new Array();
				...
				$("#add_new tbody tr").each(function(){
					...
					$(this).find('.LINCLS,.ITEMNM1,.ITEMCD,.CAQTY,.IRQTY,.SALPRC,.ACCNO,.TAXKB,.ITEMNM2,.BAQTY,.UNIT,.PRC.BIKO2,.BIKO1').each(function() {
						if($(this).attr('class')){
				...
				arr2={};
				$('.accepth').find('.ACCNO,.ESTNO,.CUSTCD,.CUSTCD3,.CUSTNM3,.CUATACCNO,.ACCDT,.RETURNDT,.SHIDT,.PERSONCD,.BIKO,.ENDKB').each(function() {
						if($(this).attr('name')){
		...
		if(ITEMCD.length===10){
			$.get("../../item/index/"+ITEMCD,
			  function(data){
			    ...
			    $(this).closest(".ITEMNM1").html(data.test);
			    alert(data.test);
			  }
			);
		}
